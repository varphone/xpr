...
XPR_API int XPR_JSON_DumpFileName(XPR_JSON* json, const char* fileName)
{
    if (!json || !fileName)
        return -1;
    return json_dump_file((json_t*)json, fileName,
                          JSON_ENCODE_ANY | JSON_INDENT(4) | JSON_SORT_KEYS);
}
...
XPR_API int XPR_JSON_DumpFileStream(XPR_JSON* json, FILE* fileStream)
{
    if (!json || !fileStream)
        return -1;
    return json_dumpf((json_t*)json, fileStream,
                      JSON_ENCODE_ANY | JSON_INDENT(4) | JSON_SORT_KEYS);
}
    ...
    if (xps->last_key[0] == 0) {
        ...
        if (bb && ai != -1) {
            if (xps->created && xps->target == NULL) {
                XPR_JSON_ObjectSetNew(xps->root, (const char*)xps->last_key,
                                      XPR_JSON_Array());
                xps->target =
                    XPR_JSON_ObjectGet(xps->root, (const char*)xps->last_key);
            }
            xps->root = xps->target;
            ...
            if (xps->created && xps->target == NULL) {
                for (i = 0; i <= ai; i++) {
                    if (XPR_JSON_ArrayGet(xps->root, i) == NULL)
                        XPR_JSON_ArrayInsertNew(xps->root, i,
                                                XPR_JSON_Object());
                }
                xps->target = XPR_JSON_ArrayGet(xps->root, ai);
            }
            ...
            if (xps->created && xps->target == NULL) {
                XPR_JSON_ObjectSetNew(xps->root, xps->last_key,
                                      XPR_JSON_Object());
            }
            xps->target =
                XPR_JSON_ObjectGet(xps->root, (const char*)xps->last_key);
...
XPR_API int XPR_JSON_XPathSet(XPR_JSON* json, const char* xpath,
                              XPR_JSON* value)
{
    struct xpath_scan xps;
    xps.created = 1;
...
XPR_API int XPR_JSON_XPathSetNew(XPR_JSON* json, const char* xpath,
                                 XPR_JSON* value)
{
    struct xpath_scan xps;
    xps.created = 1;
...
XPR_API int XPR_JSON_XPathSetBoolean(XPR_JSON* json, const char* xpath,
                                     int value)
{
    return XPR_JSON_XPathSetNew(json, xpath,
                                value ? XPR_JSON_True() : XPR_JSON_False());
}
