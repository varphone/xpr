 ...
 * multiple of 4.
 */
static void Decode(uint32_t* output, unsigned char const* input,
                   unsigned int len)
{
    unsigned int i, j;
 ...
 **/
XPR_API void XPR_MD5Update(XPR_MD5Context* context, const uint8_t* input,
                           size_t inputLen)
{
...
XPR_API void XPR_MD5Final(uint8_t digest[16], XPR_MD5Context* context)
{
    ...
    XPR_MD5Update(context, PADDING, padLen);

    /* Append length (before padding) */
    XPR_MD5Update(context, bits, 8);
...
XPR_API char* XPR_MD5End(XPR_MD5Context* ctx, char* buf)
{
    ...
    if (!buf)
        return 0;
    XPR_MD5Final(digest, ctx);
    for (i = 0; i < MD5_STRING_LENGTH; i++) {
