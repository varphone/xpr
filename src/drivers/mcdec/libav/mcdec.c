...
{
    return &xpr_mcdec_ports[XPR_MCDEC_PORT_MAJOR(port)]
                           [XPR_MCDEC_PORT_MINOR(port)];
}
...
{
    PortContext* pc = GetPortContext(port);
    return (pc->cur_fourcc != 0 && pc->cur_fourcc != AV_FOURCC_NULL &&
...
static int AVFrameToImage(int port, const XPR_AVFrame* frm)
{
    ...
    if (pc->avctx) {
        ...
        //
        if (port == XPR_MCDEC_SPEC_BMPENC_PORT &&
            frm->format != pc->avctx->pix_fmt)
            force_scaled = 1;

        if ((pc->params.image_width == 0 || pc->params.image_height == 0) &&
            !force_scaled) {
            pc->avctx->width = frm->width;
            pc->avctx->height = frm->height;
        }
        else {
            pc->avctx->width = pc->params.image_width ? pc->params.image_width
                                                      : frm->width;
            pc->avctx->height =
                pc->params.image_height ? pc->params.image_height : frm->height;
            dst_pix_fmt = port == XPR_MCDEC_SPEC_BMPENC_PORT ? AV_PIX_FMT_BGR24
                                                             : avfrm.format;
            swsctx = sws_getContext(
                avfrm.width, avfrm.height, (enum AVPixelFormat)avfrm.format,
                pc->avctx->width, pc->avctx->height,
                (enum AVPixelFormat)dst_pix_fmt, SWS_BICUBIC, NULL, NULL, NULL);
            if (swsctx) {
                if (av_image_alloc(pointers, linesizes, pc->avctx->width,
                                   pc->avctx->height,
                                   (enum AVPixelFormat)dst_pix_fmt, 4) > 0) {
                    if (sws_scale(swsctx, (const uint8_t* const*)avfrm.data,
                                  avfrm.linesize, 0, avfrm.height, pointers,
                                  linesizes) > 0)
                        scaled = 1;
                    else
                        av_freep(&pointers[0]);
...
XPR_API int XPR_MCDEC_PushStreamBlock(int port, const XPR_StreamBlock* blk)
{
    ...
    pkt.dts = AV_NOPTS_VALUE;
    pkt.pts = AV_NOPTS_VALUE;
    pkt.flags = (XPR_StreamBlockFlags(blk) & XPR_STREAMBLOCK_FLAG_TYPE_I)
                    ? AV_PKT_FLAG_KEY
                    : 0;
#if defined(WIN32) || defined(WIN64)
    __try {
...
get_frame:
    ...
    if (got) {
        if (pc->params.deinterlace) {
            memcpy(pc->out_frame.datas[0], pc->avfrm->data[0],
                   pc->avfrm->height * pc->avfrm->linesize[0]);
            memcpy(pc->out_frame.datas[1], pc->avfrm->data[1],
                   pc->avfrm->height * pc->avfrm->linesize[1]);
            memcpy(pc->out_frame.datas[2], pc->avfrm->data[2],
                   pc->avfrm->height * pc->avfrm->linesize[2]);
            ...
            memcpy(pc->out_frame.datas[0], pc->avfrm->data[0],
                   pc->avfrm->height * pc->avfrm->linesize[0]);
            memcpy(pc->out_frame.datas[1], pc->avfrm->data[1],
                   pc->avfrm->height / 2 * pc->avfrm->linesize[1]);
            memcpy(pc->out_frame.datas[2], pc->avfrm->data[2],
                   pc->avfrm->height / 2 * pc->avfrm->linesize[2]);
            pc->out_frame.pitches[0] = pc->avfrm->linesize[0];
            pc->out_frame.pitches[1] = pc->avfrm->linesize[1];
            pc->out_frame.pitches[2] = pc->avfrm->linesize[2];
            pc->out_frame.format = XPR_AV_PIX_FMT_YUV420P;
