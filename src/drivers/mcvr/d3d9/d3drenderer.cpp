...
namespace XD
{
	...
	bool D3DRenderer::resetDevice(int wnd_size)
	{
		...
		//Release textures.
		std::unordered_map<UINT, std::shared_ptr<D3DDestTexture>>::iterator tex_it;
		...
		SafeRelease(&_pixel_table);
		//Releae vertices.
		std::unordered_map<int, std::shared_ptr<D3DMainChannel>>::iterator main_it;
	...
	bool D3DRenderer::bindPort(int port, void *hwnd, int rows, int cols)
	{
		...
		std::unordered_map<int, std::shared_ptr<D3DMainChannel>>::iterator map_it;
		std::shared_ptr<D3DMainChannel> main_channel;
	...
	LRESULT D3DRenderer::respondMainHwnd(HWND hWnd, int uMsg, WPARAM wParam, LPARAM lParam)
	{
		...
		case WM_SIZE:
		{
			...
			if (D3D_DEVICE_STATE_NORMAL == _d3d_device_state) {
				...
				_wnd_size = (int)lParam;
			}
			return ret;
		}
		case WM_RESET_DEVICE:
		{
			resetDevice((int)lParam);
			return 0;
		}
		case WM_CLOSE:
	...
	LRESULT D3DRenderer::respondChannelHwnd(HWND hWnd, int uMsg, WPARAM wParam, LPARAM lParam, UINT_PTR id)
	{
		...
		case WM_CLOSE:
			if (hWnd != _main_hwnd) {
				...
				_channels_map.erase((const int)id);
			}
			break;
		default:
			break;
	...
	{
		std::shared_ptr<D3DMainChannel> main_channel;
		std::unordered_map<int, std::shared_ptr<D3DMainChannel>>::iterator map_it = _channels_map.begin();
		for (map_it = _channels_map.begin(); map_it != _channels_map.end(); ++map_it) {
			main_channel = map_it->second;
			if (main_channel.get()) {
				MAIN_CHANNEL_MISSION mission;
	...
	void D3DRenderer::loopPresenting(void)
	{
		...
		while (_is_rendering) {
			...
			if (hr != D3D_OK) {
				char msg[1024];
				sprintf_s(msg, sizeof(msg), "TestCooperativeLevel() = %08x\n", hr);
			...
			{
				OutputDebugString("D3D_DEVICE_STATE_INVALID\n");
	...
	std::shared_ptr<D3DDestTexture> D3DRenderer::getDestTexture(UINT params)
	{
		...
		std::unordered_map<UINT, std::shared_ptr<D3DDestTexture>>::iterator map_it;
