...
XPR_API char* XPR_MCVR_GetErrorString(void)
{
    ...
    case XPR_MCVR_ERR_RENDERER_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create renderer core.\n");
        break;
    ...
    case XPR_MCVR_ERR_DEVICE_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create device.\n");
        break;
    case XPR_MCVR_ERR_VERTEX_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create vertex buffer.\n");
        break;
    case XPR_MCVR_ERR_EFFECT_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create effect.\n");
        break;
    case XPR_MCVR_ERR_THREAD_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create thread.\n");
        break;
    case XPR_MCVR_ERR_TEXTURE_CREATE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to create texture.\n");
        break;
    case XPR_MCVR_ERR_IMAGE_ENHANCE_FAILED:
        sprintf_s(error_string, 100,
                  "MCVR:Error on failing to enhance image.\n");
        break;
...
XPR_API wchar_t* XPR_MCVR_GetErrorStringW(void)
{
    wchar_t* error_string =
        reinterpret_cast<wchar_t*>(XPR_Alloc(100 * sizeof(wchar_t)));
    if (!error_string) {
        return nullptr;
    }
...
XPR_API void XPR_MCVR_Fini(void)
{
...
XPR_API void XPR_MCVR_SetAspectRatio(float ratio)
{
...
XPR_API void XPR_MCVR_SetScale(int port, float factor)
{
    CHECK_RENDERER_NORET();
                                    ...
                                    XPR_MCVR_StringState state)
{
    ...
    case XPR_MCVR_STRING_TYPE_HINT:
        ...
        case XPR_MCVR_STRING_STATE_SHOW_1:
            ret = _mcvr_renderer->setChannelState(port,
                                                  XPR_MCVR_CHANNEL_NO_CAMERA);
            break;
        case XPR_MCVR_STRING_STATE_SHOW_2:
            ret = _mcvr_renderer->setChannelState(port,
                                                  XPR_MCVR_CHANNEL_BUFFERING);
            break;
        case XPR_MCVR_STRING_STATE_SHOW_3:
            ret = _mcvr_renderer->setChannelState(port,
                                                  XPR_MCVR_CHANNEL_INTERRUPT);
            break;
        case XPR_MCVR_STRING_STATE_SHOW_4:
            ret =
                _mcvr_renderer->setChannelState(port, XPR_MCVR_CHANNEL_STOPPED);
            break;
                                                     ...
                                                     XPR_MCVR_StringType type)
{
    ...
    switch (type) {
    case XPR_MCVR_STRING_TYPE_TITLE:
        return 0 < _mcvr_renderer->getTitleFlag() ? XPR_MCVR_STRING_STATE_SHOW
                                                  : XPR_MCVR_STRING_STATE_HIDE;
    case XPR_MCVR_STRING_TYPE_HINT: {
