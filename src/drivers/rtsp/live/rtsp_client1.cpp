#if defined(XPR_RTSP_DRIVER_LIVE) && defined(HAVE_XPR_RTSP_CLIENT1)
...
typedef struct Callback {
    ...
    void* evcb_opaque;
#ifdef HAVE_XD_STREAM_API
    XD_StreamDataCallback dcb_xd;
    void* dcb_opaque_xd;
    XD_StreamEventCallback evcb_xd;
    void* evcb_opaque_xd;
#endif
} Callback;
...
#ifdef HAVE_HKMI
    HIK_MEDIAINFO out_pes_hkmi;
#endif

	XPR_RTSP_TRSPEC trspec; ///< ´«Êä·½Ê½
...
typedef struct XPR_RTSP {
    int exit_loop;
    PortContext ports[XPR_RTSP_MAX_CLIENTS+2];
...
static PortContext* GetPortContext(int port)
{
    ...
    if (minor > 0 && minor <= XPR_RTSP_MAX_CLIENTS)
        return &xpr_rtsp->ports[minor];
    return 0;
}

static int GetAvailPortIndex(void)
{
    for (int i = 1; i <= XPR_RTSP_MAX_CLIENTS; i++) {
        if (xpr_rtsp->ports[i].usr_flags == 0 &&
            xpr_rtsp->ports[i].act_flags == 0)
            return i;
    }
    return -1;
}
...
static void UpdatePortLATS(int port)
{
...
static void UpdatePortLVTS(int port)
{
    PortContext* pc = GetPortContext(port);
    if (pc) {
...
void continueAfterDESCRIBE(RTSPClient* rtspClient, int resultCode, char* resultString)
{
    do {
        ...
        UpdatePortLATS(((ourRTSPClient*)rtspClient)->port);
        return;
    } while (0);
...
void setupNextSubsession(RTSPClient* rtspClient)
{
	...
	PortContext* pc = GetPortContext(((ourRTSPClient*)rtspClient)->port);
...
void continueAfterSETUP(RTSPClient* rtspClient, int resultCode, char* resultString) {
    do {
        ...
        scs.subsession->sink = DummySink::createNew(env, (ourRTSPClient*)rtspClient, *scs.subsession, ((ourRTSPClient*)rtspClient)->port, scs.lastTrackId);
...
void continueAfterGET_PARAMETER(RTSPClient* rtspClient, int resultCode, char* resultString)
{
    if (rtspClient) {
        ...
        UpdatePortLVTS(((ourRTSPClient*)rtspClient)->port);
    }
}
...
ourRTSPClient* ourRTSPClient::createNew(UsageEnvironment& env, int port)
{
    ...
    if (xu) {
        if (XPR_UrlGetPort(xu) != 0)
            sprintf(url, "rtsp://%s:%d/%s", XPR_UrlGetHost(xu), XPR_UrlGetPort(xu), XPR_UrlGetPath(xu));
        else
...
DummySink* DummySink::createNew(UsageEnvironment& env, ourRTSPClient* client, MediaSubsession& subsession, int port, int trackId) {
    return new DummySink(env, client, subsession, port, trackId);
}

DummySink::DummySink(UsageEnvironment& env, ourRTSPClient* client, MediaSubsession& subsession, int port, int trackId)
    : MediaSink(env)
    , fClient(client)
    , fPTS(0)
...
static int H264_DetectSliceType(const uint8_t* data, size_t size)
{
    ...
    if (slice_type_zn) {
        ...
        slice_type = (b >> (n - first_mb_in_slice_zn * 2 - 1 - slice_type_zn * 2 - 1)) - 1;
    }
    //
    slice_type &= 0xf;
    //printf("slice_type: %d\n", slice_type);
...
struct timeval presentationTime, unsigned /*durationInMicroseconds*/) {
	...
	PortContext* pc = GetPortContext(fPort);
	uint8_t* ptr = NULL;
        ...
        if (fCodec == AV_FOURCC_H264) {
            ...
            if (!(fReceiveBuffer[0] == 0 &&
				...
				if (fFrameType == 0 && naluType < 6) {
					fFrameType = H264_SliceTypeToFrameType(H264_DetectSliceType(fReceiveBuffer, frameSize));
				}
				ptr = fReceiveBuffer;
            }
            else {
                ...
                if (fFrameType == 0 && naluType < 6) {
                    fFrameType = H264_SliceTypeToFrameType(H264_DetectSliceType(fReceiveBuffer+4, frameSize - 4));
                }
				if (!(pc->h264_flags & H264_FLAG_START_CODE)) {
...
static void* ThreadLoop(void* opaque, XPR_Thread* thread)
{
    ...
    while (!xpr_rtsp->exit_loop) {
        ...
        if ((cnt % XPR_RTSP_MAX_WORKERS) == idx) {
            int port = XPR_RTSP_PORT(XPR_RTSP_PORT_MAJOR_CLI, (cnt & 0xff), 0);
            PortContext* pc = GetPortContext(port);
            if (pc) {
                if ((pc->act_flags == 0) &&
                    ...
                    pc->url[0] != 0) {
                        ...
                        pc->out_pes = 0;
                        pc->rtsp_client = openURL(*env, port);
                    ...
                    if (pc->rtsp_client) {
                        ...
                        shutdownStream((RTSPClient*)pc->rtsp_client);
                    }
                    else {
                        pc->act_flags &= ~XPR_RTSP_FLAG_STOP;
                        pc->rtsp_client = 0;
                    ...
                    pc->act_flags |= XPR_RTSP_FLAG_KPL;
                    keepAlive((RTSPClient*)pc->rtsp_client);
                }
                //
...
int XPR_RTSP_Open(int port, const char* url)
{
    ...
    if (minor == XPR_RTSP_PORT_MINOR_ANY) {
        minor = GetAvailPortIndex();
        if (minor < 0)
            return -1;
        port = XPR_RTSP_PORT(XPR_RTSP_PORT_MAJOR_CLI, minor, 0);
    }
...
static int PESPacketHandler(const uint8_t* data, int length, void* opaque)
{
    ...
    stb.flags |= (pc->out_pes_stb->codec == AV_FOURCC_PCMA || pc->out_pes_stb->codec == AV_FOURCC_PCMU) ? XPR_STREAMBLOCK_FLAG_AUDIO_FRAME : 0;
    stb.flags |= (pc->out_pes_stb->codec == AV_FOURCC_H264 || pc->out_pes_stb->codec == AV_FOURCC_JPEG) ? XPR_STREAMBLOCK_FLAG_VIDEO_FRAME : 0;
...
#endif /* defined(XPR_RTSP_DRIVER_LIVE) && defined(HAVE_XPR_RTSP_CLIENT1) */