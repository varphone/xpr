...
static void InitBase64DecodeTable(void)
{
    int i;
    for (i = 0; i < 256; ++i)
        gBase64DecodeTable[i] = (char)0x80;
    // default value: invalid

    for (i = 'A'; i <= 'Z'; ++i)
        gBase64DecodeTable[i] = 0 + (i - 'A');
    for (i = 'a'; i <= 'z'; ++i)
        gBase64DecodeTable[i] = 26 + (i - 'a');
    for (i = '0'; i <= '9'; ++i)
        gBase64DecodeTable[i] = 52 + (i - '0');
...
XPR_API char* XPR_Base64Encode(const unsigned char* origSigned,
                               unsigned int origLength)
{
    const unsigned char* orig = (const unsigned char*)
    ...
    for (i = 0; i < numOrig24BitValues; ++i) {
        ...
        result[4 * i + 1] =
            gBase64Char[(((orig[3 * i] & 0x3) << 4) | (orig[3 * i + 1] >> 4)) &
                        0x3F];
        result[4 * i + 2] =
            gBase64Char[((orig[3 * i + 1] << 2) | (orig[3 * i + 2] >> 6)) &
                        0x3F];
    ...
    if (havePadding) {
        result[4 * i + 0] = gBase64Char[(orig[3 * i] >> 2) & 0x3F];
        if (havePadding2) {
            result[4 * i + 1] = gBase64Char[(((orig[3 * i] & 0x3) << 4) |
                                             (orig[3 * i + 1] >> 4)) &
                                            0x3F];
