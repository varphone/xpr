...
static const char* xpr_icmp_echo_request_pattern = "=== XPR_ICMP ECHO REQEUST ===";
...
static int XPR_ICMP_Init(XPR_ICMP* icmp, const char* dev)
{
    ...
    if (setsockopt(icmp->sock, SOL_SOCKET, SO_SNDTIMEO, &tmo, sizeof(tmo)) < 0) {    
        return XPR_ERR_ERROR;    
    }
    if (setsockopt(icmp->sock, SOL_SOCKET, SO_RCVTIMEO, &tmo, sizeof(tmo)) < 0) {    
        return XPR_ERR_ERROR;
    }
    return XPR_ERR_OK;
}
...
int XPR_ICMP_EchoRequest(XPR_ICMP* icmp, const char* host)
{
    int n = 0;
    pid_t pid = -1;
    ...
    memcpy(icmp_pkt->icmp_data + 8, xpr_icmp_echo_request_pattern, strlen(xpr_icmp_echo_request_pattern));
    icmp_pkt->icmp_cksum = inet_chksum((unsigned char*)icmp_pkt, sizeof(icmp->tx_buf));

    memset(&to, 0, sizeof(to));
    to.sin_family = AF_INET;
    to.sin_addr.s_addr = inet_addr(host);

    n = sendto(icmp->sock, icmp->tx_buf, sizeof(icmp->tx_buf), 0, (struct sockaddr*)&to, sizeof(to));

    return n > 0 ? XPR_ERR_OK : XPR_ERR_ERROR;
}

int XPR_ICMP_Poll(XPR_ICMP* icmp)
{
    int n = 0;
