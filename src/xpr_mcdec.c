...
static int ResetPort(int port, uint32_t fourcc)
{
	...
	if (major == XPR_MCDEC_PORT_MAJOR_GEN) {
		...
		pc->avctx->thread_count = 1;;
		pc->avctx->thread_type = 0;//FF_THREAD_FRAME | FF_THREAD_SLICE;
        ...
        if ((pc->params.image_width == 0 || pc->params.image_height == 0) && !force_scaled) {
            ...
            if (swsctx) {
                if (avpicture_alloc(&avpic, (enum AVPixelFormat)dst_pix_fmt, pc->avctx->width, pc->avctx->height) == 0) {
                    if (sws_scale(swsctx, (const uint8_t * const*)avfrm.data, avfrm.linesize, 0, avfrm.height, avpic.data, avpic.linesize) > 0)
                        scaled = 1;
                    else
