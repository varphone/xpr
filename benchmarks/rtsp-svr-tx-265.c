...
const char* exampleFileName = "example.265.framed";
...
void benchmark(void)
{
    ...
    // - mime - Track data mime type, like: audio/AAC, video/H265
    err = XPR_RTSP_Open(streamPort, "uri:///example.265"
                                    "?track=1&mime=video/H265&appendOriginPTS=0");
    XPR_ERR_ASSERT(err);
    fprintf(stderr, "RTSP [%d] URL: %s\n", streamPort,
            "rtsp://0.0.0.0:8554/example.265");
